...
window.SorusNative = {
  ...
  detailsExists: ipc.sendSync('does-details-exist'),
}

// FIXME: GitHub doesn't return the file (or even metadata) for files larger than 1 MB. Also nice spaghetti.

if (!existsSync(join(SorusNative.userData, 'mc', 'Sorus', 'client', 'Core.jar')) ||
    !existsSync(join(SorusNative.userData, 'mc', 'Sorus', 'client', SorusNative.options.mc_ver + '.jar')) ||
    !existsSync(join(SorusNative.userData, 'mc', 'Sorus', 'client', 'JavaAgent.jar')))
